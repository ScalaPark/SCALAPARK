...
import scala.ecommerce.services.OrderGenerator
...
class OrderController @Inject() (
  ...
  def createOrder() = Action:
    Future {
      orderGenerator.generateBatch().zipWithIndex.foreach { case ((order, isValid), i) =>
        val index = i + 1
        ...
        if index < orderGenerator.totalOrdersToSend then
          Thread.sleep(intervalSeconds * 1000L)
      }
    }

    Accepted(
      s"Batch started: sending ${orderGenerator.totalOrdersToSend} orders every $intervalSeconds seconds."
    )
